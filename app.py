...
def show_question(question_id):
    ...
    try:
        correct_answers_parsed = json.loads(question.correct_answer)
        is_multi_select_question = isinstance(correct_answers_parsed, list) and len(correct_answers_parsed) > 1
    except json.JSONDecodeError:
        is_multi_select_question = False
...
def review_incorrect():
    user_id = current_user.id
    incorrect_answers_raw = UserAnswer.query.filter_by(user_id=user_id, is_correct=False)\
                                           .order_by(UserAnswer.timestamp.desc()).all()
    ...
    for ua in incorrect_answers_raw:
        if ua.question_id not in seen_question_ids:
            if ua.question_detail:
                user_selected_display = " & ".join(json.loads(ua.user_selected_option)) if ua.user_selected_option else ""
                correct_answer_display = " & ".join(json.loads(ua.question_detail.correct_answer)) if ua.question_detail.correct_answer else ""
...
def my_checked_questions_by_type(check_type):
    ...
    for entry in checked_entries:
        if entry.question_detail:
            correct_answer_display = " & ".join(json.loads(entry.question_detail.correct_answer)) if entry.question_detail.correct_answer else ""
